...
// Deployed to Polygon Mumbai 0x8375dcBE8e6e3c39E72833B8CA0b42E314789493
...
contract funder is KeeperCompatibleInterface, soulbound {
    //State variables
    uint TotalVotes;
    uint votesRequired;
    uint votingTime;
    ...
    mapping(bytes32 => mapping(address => bool)) Claimed_Nft; // user claimed for a specific program
        ...
        payable
    {
        ...
        if (fundGoal >= 1000 && fundGoal < 10000) {
            votesRequired = 2000;
            votingTime = 3 days;
            RaisingFundsTime = 5 * 30 days;
        } else if (fundGoal >= 10000 && fundGoal < 100000) {
            votesRequired = 5000;
            votingTime = 3 days;
            RaisingFundsTime = 1 * 365 days;
        } else if (fundGoal >= 100000) {
            votesRequired = 10000;
            votingTime = 3 days;
            RaisingFundsTime = 3 * 365 days;
        } else {
            votesRequired = 1000;
            votingTime = 2 days;
            RaisingFundsTime = 1 * 30 days;
        }
